...
BIN_PHANTOMJS=./node_modules/phantomjs/bin/phantomjs
BIN_SLIMERJS=./node_modules/slimerjs/bin/slimerjs
...
echo "PhantomJS" && $BIN_PHANTOMJS --cookies-file=$COOKIES $JS_SCRIPT "$TARGET_URL" "$ACTION" "$SUBACTION" "$FIRST_DOMAIN" "$THIRD_DOMAIN"
echo "SlimerJS" && $BIN_SLIMERJS $JS_SCRIPT "$TARGET_URL" "$ACTION" "$SUBACTION" "$FIRST_DOMAIN" "$THIRD_DOMAIN"
